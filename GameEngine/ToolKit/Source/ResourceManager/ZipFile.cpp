...
// --------------------------------------------------------------------------
std::string ZipFile::getFilename(int p_I)  const
{
	std::string fileName = "";
	if (p_I >= 0 && p_I < m_NumEntries)
	{
		char pszDest[_MAX_PATH];
		memcpy(pszDest, m_AppDir[p_I]->GetName(), m_AppDir[p_I]->fnameLen);
		pszDest[m_AppDir[p_I]->fnameLen] = '\0';
		fileName = pszDest;
	}
	return fileName;
}
...
// --------------------------------------------------------------------------
int ZipFile::getFileLen(int p_I) const
{
	if (p_I < 0 || p_I >= m_NumEntries)
		return -1;
	else
		return m_AppDir[p_I]->ucSize;
}
...
// --------------------------------------------------------------------------
bool ZipFile::readFile(int p_I, void *p_Buffer)
{
	if (p_Buffer == NULL || p_I < 0 || p_I >= m_NumEntries)
		return false;
	...
	fseek(m_File, m_AppDir[p_I]->hdrOffset, SEEK_SET);
	TZipLocalHeader h;
...
// Parameters:    The file index and the pre-allocated buffer
// --------------------------------------------------------------------------
bool ZipFile::readLargeFile(int p_I, void *p_Buffer, void(*progressCallback)(int, bool &))
{
	if (p_Buffer == NULL || p_I < 0 || p_I >= m_NumEntries)
		return false;
